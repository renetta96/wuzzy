...
local DOTable = Class(function(self, inst)
	self.inst = inst
	self.stacks = {}
	...
	self.inst:ListenForEvent("death", function(inst) self:StopTicking() end)
	self.inst:ListenForEvent("onremove", function(inst) self:StopTicking() end)
end)
...
function DOTable:Add(source, damage, numticks)
	...
	if numticks > 0 then
		...
		table.insert(self.stacks[source], {
			damage = damage,
			ticksleft = numticks,
		})
