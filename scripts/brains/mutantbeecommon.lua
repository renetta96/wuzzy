...
local function GoHomeAction(inst)
  ...
  if
    homeseeker and homeseeker.home and homeseeker.home:IsValid() and homeseeker.home.components.childspawner and
      (not homeseeker.home.components.burnable or not homeseeker.home.components.burnable:IsBurning())
   then
...
local function DespawnAction(inst)
  ...
  if
    follower and follower.leader and follower.leader:IsValid() and follower.leader.components.health and
      not follower.leader.components.health:IsDead()
   then
...
local function canareaattack(epic)
  ...
  return epic.components.combat and epic.components.combat.areahitdamagepercent ~= nil and
...
local function IsEpicAttackComing(inst, dist)
  ...
  if canareaattack(epic) then
    ...
    if
      epic.components.combat.laststartattacktime ~= nil and
        epic.components.combat.laststartattacktime + estimated_epic_atk_time >= GetTime()
     then
      return true
    end
...
local function AvoidEpicAtkNode(inst, dist)
  return WhileNode(
    ...
    RunAway(
      inst,
      function()
        return FindEpicEnemy(inst, dist)
      end,
      AVOID_EPIC_DIST,
      STOP_AVOID_EPIC_DIST
    )
  )
end
...
local CircleAroundTarget =
  Class(
  BehaviourNode,
  function(self, inst, radius, avoid_radius, thetaincrement, max_time)
