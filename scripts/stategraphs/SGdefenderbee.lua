...
local events = {
  ...
  EventHandler(
    ...
    function(inst)
      if
        (not inst.sg:HasStateTag("busy") or inst.sg:HasStateTag("caninterrupt")) and not inst.components.health:IsDead()
       then
  ...
  State {
    ...
    onupdate = function(inst)
      if inst.sg.statemem.queen == nil then
        inst.Physics:Stop()
      elseif inst.sg.statemem.queen:IsValid() and inst:IsNear(inst.sg.statemem.queen, TUNING.BEEGUARD_GUARD_RANGE) then
        inst.Physics:SetMotorVel(2, 0, 0)
      else
